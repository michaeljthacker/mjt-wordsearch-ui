...
- B2: Wired MJT Analytics — direct static import from `https://analytics.mjt.pub/analytics.js`, `init()` + `pageView()` on load, `track()` on Generate, Copy Share Link, Print, New Puzzle, and Show Solution.
