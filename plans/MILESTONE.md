...
**Acceptance:**
- [ ] Entering duplicate words deduplicates them silently (or warns) before generation — no duplicate entries reach the generator
- [ ] User-entered words that exactly match a banned word (e.g., "HELL") are accepted without error
- [ ] User-entered words containing a banned substring (e.g., "SHELL" containing "HELL") are accepted without error
- [ ] Filler-letter profanity scan continues to block accidental banned words in rows, columns, and diagonals
- [ ] `encodePayload` chunks the `String.fromCodePoint` call so it does not exceed the JS argument limit for large word lists
- [ ] A word list exceeding 20 words shows a user-facing error before generation
...
**Acceptance:**
- [ ] Grid scales gracefully on viewports < 480px (cells shrink or container scrolls horizontally) without layout breakage
- [ ] Favicon files in `favicon/` are linked from `index.html` (ico, apple-touch-icon, webmanifest); renders in browser tabs
- [ ] `<meta>` Open Graph tags (`og:title`, `og:description`, `og:url`) are set in `index.html`
- [ ] A "New Puzzle" button appears after generation and resets the view to the blank form
- [ ] Finding all words displays a visible congratulatory message (e.g., banner or styled text)
...
**Acceptance:**
- [ ] A `CNAME` file exists in the repo root containing `wordsearch.mjt.pub`
- [ ] GitHub Pages is configured to serve from `main` branch
- [ ] The app is reachable at `https://wordsearch.mjt.pub` (after human completes DNS)
- [ ] Generating a puzzle, copying a share link, and opening it in a new browser tab reproduces the same puzzle
- [ ] Print preview shows the expected clean layout
- [ ] Solve and solution toggle work correctly on the deployed site
